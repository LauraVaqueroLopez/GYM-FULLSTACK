...
-- ============================================
-- Triggers para DetallePedidos
-- ============================================
DELIMITER $$

-- BEFORE INSERT: Sumar cantidades si el producto ya existe
CREATE TRIGGER detallepedidos_before_insert
BEFORE INSERT ON DetallePedidos
FOR EACH ROW
BEGIN
    DECLARE existing_id CHAR(36);
    DECLARE existing_cant INT;
    
    -- Revisar si ya existe
    SELECT id_detalle, cantidad INTO existing_id, existing_cant
    FROM DetallePedidos
    WHERE id_pedido = NEW.id_pedido AND id_producto = NEW.id_producto
    LIMIT 1;
    ...
    IF existing_id IS NOT NULL THEN
        ...
        SET cantidad = cantidad + NEW.cantidad,
            subtotal = (cantidad + NEW.cantidad) * (SELECT precio FROM Productos WHERE id_producto = NEW.id_producto)
        WHERE id_detalle = existing_id;
        
        -- Cancelar inserción duplicada
        SET NEW.id_detalle = existing_id; 
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Producto ya existe, cantidad sumada automáticamente';
    ELSE
        -- Calcular subtotal para nuevo detalle
        SET NEW.subtotal = NEW.cantidad * (SELECT precio FROM Productos WHERE id_producto = NEW.id_producto);
    END IF;
END$$

-- AFTER INSERT/UPDATE: Actualizar total del pedido
CREATE TRIGGER detallepedidos_after_insert_update
AFTER INSERT ON DetallePedidos
FOR EACH ROW
BEGIN
    UPDATE Pedidos p
    SET total = (SELECT COALESCE(SUM(subtotal),0) FROM DetallePedidos WHERE id_pedido = p.id_pedido)
